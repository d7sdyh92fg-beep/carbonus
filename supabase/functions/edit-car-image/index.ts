  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 402) {
        return new Response(
          JSON.stringify({ error: 'Credits depleted. Please add funds to your Lovable AI workspace.' }),
    ...
    if (!editedImageUrl) {
      ...
      return new Response(
        JSON.stringify({ error: 'No edited image in response' }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    console.error('Error in edit-car-image function:', error);
    return new Response(
