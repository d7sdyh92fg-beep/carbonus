...
interface SpaceTourerDetailPageProps {
...
const GALLERY = [
  ...
  { src: spaceInterior, lt: "Citroën SpaceTourer priekinis salonas", en: "Citroën SpaceTourer front cabin", cover: true, imageClass: "" },
  { src: spacePassengerCabin, lt: "Citroën SpaceTourer keleivių salonas", en: "Citroën SpaceTourer passenger cabin", cover: true, imageClass: "" },
];
...
export function SpaceTourerDetailPage({ pricing, selectedPackage, onSelectedPackageChange }: SpaceTourerDetailPageProps) {
  ...
  const copy = isEnglish
    ? {
        premium: "Spacious minivan",
        title: "Citroën SpaceTourer",
        subtitle: "Space for the whole group, comfort for every journey.",
        description:
          "A spacious eight-seat minivan with automatic transmission, generous luggage capacity and a comfortable three-row cabin. A practical choice for family holidays, group trips and longer journeys across Lithuania.",
        ...
        highlightTitle: "More space for shared journeys",
        highlightItems: ["8 comfortable seats", "Flexible three-row cabin", "Automatic transmission"],
        bookingTitle: "Reserve your Citroën SpaceTourer",
    ...
    : {
        premium: "Erdvus vienatūris",
        title: "Citroën SpaceTourer",
        subtitle: "Vieta visai kompanijai, komfortas kiekvienoje kelionėje.",
        description:
          "Erdvus aštuonvietis vienatūris su automatine pavarų dėže, talpia bagažine ir patogiu trijų eilių salonu. Praktiškas pasirinkimas šeimos atostogoms, grupinėms išvykoms ir ilgesnėms kelionėms po Lietuvą.",
        ...
        highlightTitle: "Daugiau erdvės bendroms kelionėms",
        highlightItems: ["8 patogios vietos", "Lankstus trijų eilių salonas", "Automatinė pavarų dėžė"],
        bookingTitle: "Rezervuokite Citroën SpaceTourer",
  ...
  const features = Array.from({ length: 8 }, (_, index) => t(`carData.7.feature${index + 1}`));
  const featureGroups = [
  ...
  const specs = [
    ...
    [copy.doors, "5"],
    [copy.trunk, "2763 L (max)"],
    [copy.engine, "2.2L Diesel 180 AG"],
  ];
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-white font-sans text-foreground">
      <SEOHead
        ...
        canonical={`https://carbonus.lt/${isEnglish ? "cars/citroen-spacetourer-rental" : "automobiliai/citroen-spacetourer-nuoma"}`}
        ogImage={spaceFront}
        ogType="product"
      ...
      <main className="pt-[78px]">
        <section className="bg-[hsl(210_20%_96%)] pb-10 pt-7 sm:pb-12 sm:pt-9">
          <div className="mx-auto max-w-[1320px] px-5 sm:px-6">
            <nav className="flex flex-wrap items-center gap-2 text-[13px] font-semibold uppercase tracking-[0.07em] text-foreground/60" aria-label="Breadcrumb">
              ...
              <span>/</span>
              <span className="text-[hsl(var(--carbonus-green-dark))]">Citroën SpaceTourer</span>
            </nav>
            ...
            <div className="mt-7 grid gap-8 lg:grid-cols-[minmax(0,1.14fr)_minmax(430px,0.86fr)] lg:items-start lg:gap-8">
              <div>
                <div className="relative overflow-hidden rounded-[28px] border border-white bg-white shadow-[0_24px_60px_rgba(15,23,42,0.09)]">
                  <div className="relative flex h-[350px] items-center justify-center overflow-hidden bg-white sm:h-[500px] lg:h-[560px]">
                    ...
                    />
                    <button onClick={showPrevious} aria-label={isEnglish ? "Previous photo" : "Ankstesnė nuotrauka"} className="absolute left-4 top-1/2 flex h-12 w-12 -translate-y-1/2 items-center justify-center rounded-full border border-transparent bg-transparent text-[hsl(var(--carbonus-green-dark))] drop-shadow-[0_1px_2px_rgba(255,255,255,0.95)] transition hover:scale-110 hover:bg-[hsl(var(--carbonus-green)/0.08)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[hsl(var(--carbonus-green))]">
                      <ChevronLeft className="h-5 w-5" />
                    </button>
                    <button onClick={showNext} aria-label={isEnglish ? "Next photo" : "Kita nuotrauka"} className="absolute right-4 top-1/2 flex h-12 w-12 -translate-y-1/2 items-center justify-center rounded-full border border-transparent bg-transparent text-[hsl(var(--carbonus-green-dark))] drop-shadow-[0_1px_2px_rgba(255,255,255,0.95)] transition hover:scale-110 hover:bg-[hsl(var(--carbonus-green)/0.08)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[hsl(var(--carbonus-green))]">
                ...
                <div className="mt-5 grid grid-cols-2 gap-4 sm:grid-cols-4">
                  {GALLERY.map((image, index) => (
                    ...
                    >
                      <img src={image.src} alt="" width={320} height={220} loading="lazy" decoding="async" className={`h-full w-full ${image.cover ? "object-cover" : "object-contain p-1 sm:scale-[1.15]"}`} />
                      {index >= 2 && (
              ...
              <aside className="rounded-[28px] border border-border/70 bg-white p-7 shadow-[0_22px_55px_rgba(15,23,42,0.08)] sm:p-9 lg:sticky lg:top-[98px]">
                ...
                <div className="mt-6 grid grid-cols-3 gap-2">
                  {[
                    { Icon: Users, value: "8", label: copy.passengers },
            ...
            <div className="mt-20 grid gap-7 sm:mt-24 lg:grid-cols-[0.82fr_1.18fr] lg:items-start">
              <div className="order-last lg:order-first">
                <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-[hsl(var(--carbonus-green-dark))]">Citroën SpaceTourer</p>
              ...
              <div className="order-first aspect-[16/10] w-full overflow-hidden rounded-[24px] bg-[#eee7dc] shadow-[0_18px_45px_rgba(15,23,42,0.12)] lg:order-last">
                <img src={spaceScenic} alt={isEnglish ? "Citroën SpaceTourer scenic drive" : "Citroën SpaceTourer kelionėje"} width={1536} height={1024} loading="lazy" decoding="async" className="h-full w-full object-cover object-center" />
              </div>
            </div>
          </div>
        </section>
        ...
        <section id="booking-section" className="scroll-mt-24 bg-[hsl(210_20%_97%)] py-16 sm:py-[72px]">
          <div className="mx-auto max-w-[1320px] px-5 sm:px-6">
            ...
            <BookingCalendar carId="7" carName="Citroën SpaceTourer" carImage={spaceFront} selectedPackage={selectedPackage} onClearPackage={() => onSelectedPackageChange(null)} onDatesChange={setLiveSelectedDates} />
          </div>
        </section>
